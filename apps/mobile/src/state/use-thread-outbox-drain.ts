  ...
  const sendQueuedMessage = useCallback(
    async (queuedMessage: QueuedThreadMessage, thread: EnvironmentThreadShell) => {
      ...
      if (failure?.action === "discard") {
        const removed = await removeThreadOutboxMessage(persistedMessage, deliveryRevision);
        if (removed) {
          await prepared.releaseUploads().catch((error) => {
            console.warn("[thread-outbox] could not delete discarded pending uploads", error);
          });
        }
        return removed;
      }
